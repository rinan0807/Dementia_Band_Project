...
// ================================================================================
#define CAREGIVER_TEST_NUMBER   "+91XXXXXXXXXX"   // YOUR phone number, with country code
...
bool coreTestsOK = true; // tracks modem/SIM/network/SMS - the tests that matter for actual function
...
void setup() {
  ...
  Serial.println(F("========================================"));
  Serial.println();
  Serial.println(coreTestsOK
    ? F("LED will now blink SLOWLY (~1x/sec): modem/SIM/network/SMS all OK.")
    : F("LED will now blink FAST (~3x/sec): one or more of modem/SIM/network/SMS failed."));
  Serial.println(F("This lets you unplug USB and still see a pass/fail result with no Serial Monitor."));
}
...
void loop() {
  ...
  // Status LED, works with no USB/Serial Monitor connected at all:
  // slow blink = modem/SIM/network/SMS all passed, fast blink = something failed.
  if (coreTestsOK) {
